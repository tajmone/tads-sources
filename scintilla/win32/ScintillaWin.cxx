...
#include <richedit.h>
#include <windowsx.h>
#include <shlobj.h> // [mjr]

#if defined(_MSC_VER) && (_MSC_VER > 1200)
//#define USE_D2D 1 [mjr - commented out]
#endif
...
class ScintillaWin :
        ...
        bool hasOKText;
//        bool hasOKFile; // [MJR - for windows explorer file drop support]
        ...
        DataObject dob;
        DropTarget dt;
        IDropTargetHelper *dthelper; // [MJR]
        ...
        virtual PRectangle GetClientRectangle(); // [MJR]
...
private:
        ...
        HWND GetTrueParent(); //[mjr]
        BOOL IsWindowMaximized(HWND hwnd); //[mjr]
        HBITMAP sysCaretBitmap;
        int sysCaretWidth;
        int sysCaretHeight;
        bool keysAlwaysUnicode;

        // scrollbars, for when we manage them as explicit separate controls [MJR]
        HWND vscroll;
        HWND hscroll;
        HWND sizegrip;
        HWND graybox;
...
//        hasOKFile = false; // [mjr]
        ...
        // create scrollbars if our window doesn't have any [MJR]
        if ((::GetClassLong(hwnd, GCL_STYLE) & (WS_HSCROLL | WS_VSCROLL)) == 0) {
                vscroll = ::CreateWindow("SCROLLBAR", "",
                                         WS_CHILD | WS_VISIBLE | SBS_VERT
                                         | SBS_RIGHTALIGN,
                                         0, 0, ::GetSystemMetrics(SM_CXVSCROLL), 1,
                                         hwnd, 0, 0, 0);
                hscroll = ::CreateWindow("SCROLLBAR", "",
                                         WS_CHILD | WS_VISIBLE | SBS_HORZ
                                         | SBS_BOTTOMALIGN,
                                         0, 0, 1, ::GetSystemMetrics(SM_CYHSCROLL),
                                         hwnd, 0, 0, 0);
                sizegrip = ::CreateWindow("SCROLLBAR", "",
                                          WS_CHILD | WS_VISIBLE
                                          | SBS_SIZEBOX | SBS_SIZEGRIP
                                          | SBS_SIZEBOXBOTTOMRIGHTALIGN,
                                          0, 0, 1, 1, hwnd, 0, 0, 0);
                graybox = ::CreateWindowEx(WS_EX_WINDOWEDGE, "STATIC", "",
                                           WS_CHILD | SS_NOTIFY,
                                           0, 0, 1, 1, hwnd, 0, 0, 0);
        } else {
                // we don't need separate scrollbars
                hscroll = vscroll = sizegrip = graybox = 0;
        }
        ...
        try {
                ...
                case WM_CREATE:
                        ...
                        CoCreateInstance(CLSID_DragDropHelper, 0, CLSCTX_INPROC_SERVER,
                                         IID_IDropTargetHelper, (LPVOID *)&dthelper);
                        // [mjr] end
                        break;
                ...
                case WM_SIZE: {
                        ...
                        if (vscroll != 0) {
                                ...
                                ::MoveWindow(vscroll, rc.right - wid, 0,
                                             wid, rc.bottom - ht, TRUE);
                                ::MoveWindow(hscroll, 0, rc.bottom - ht,
                                             rc.right - wid, ht, TRUE);
                                ::MoveWindow(sizegrip, rc.right - wid,
                                             rc.bottom - ht, wid, ht, TRUE);
                                ::MoveWindow(graybox, rc.right - wid,
                                             rc.bottom - ht, wid, ht, TRUE);

                                // show the size grip only if we cover our parent's full area;
                                // show the gray box in the inverse condition
                                HWND parent = GetTrueParent();
                                ::MapWindowPoints(
                                        MainHWND(), parent, (POINT *)&rc, 2);
                                ::GetClientRect(GetParent(MainHWND()), &prc);
                                int show_sizer = (rc.right == prc.right
                                        && rc.bottom == prc.bottom
                                        && !IsWindowMaximized(parent));
                                ::ShowWindow(sizegrip, show_sizer
                                             ? SW_SHOW : SW_HIDE);
                                ::ShowWindow(graybox, show_sizer
                                             ? SW_HIDE : SW_SHOW);
                        }
                        break;
                }
                ...
                case WM_RBUTTONUP:
                        // in case we tracked the mouse-down, handle the mouse-up as well
                        ButtonUp(Point::FromLong(lParam),
                                 ::GetMessageTime(),
                                 (wParam & MK_CONTROL) != 0);

                        // ...and then also do the normal Windows work to bring up the menu
                ...
                case WM_RBUTTONDOWN:
                        // begin [mjr]
                        // if the click isn't in the selection and isn't in the margin,
                        // start a new selection
                        if (!PointInSelection(Point::FromLong(lParam))
                            && !PointInSelMargin(Point::FromLong(lParam))) {
                                ButtonDown(Point::FromLong(lParam), ::GetMessageTime(),
                                           (wParam & MK_SHIFT) != 0,
                                           (wParam & MK_CONTROL) != 0,
                                           Platform::IsKeyDown(VK_MENU));
                ...
                case WM_SETCURSOR:
                        if (LoWord(lParam) == HTCLIENT && (HWND)wParam == MainHWND()) { //[mjr - added HWND test]
                ...
                case WM_CTLCOLORSTATIC:
                        if (graybox != 0 && (HWND)lParam == graybox) {
...
void ScintillaWin::ScrollText(int linesToMove) {
        ...
        if (vscroll != 0)
        {
                ...
                ::ScrollWindow(MainHWND(), 0,
                               vs.lineHeight * linesToMove, &rc, &rc);
        }
        else
                ::ScrollWindow(MainHWND(), 0,
                               vs.lineHeight * linesToMove, 0, 0);
...
int ScintillaWin::SetScrollInfo(int nBar, LPCSCROLLINFO lpsi, BOOL bRedraw) {
        // [MJR]
        if (nBar == SB_VERT && vscroll != 0)
                return ::SetScrollInfo(vscroll, SB_CTL, lpsi, bRedraw);
        else if (nBar == SB_HORZ && hscroll != 0)
                return ::SetScrollInfo(hscroll, SB_CTL, lpsi, bRedraw);
        else
...
bool ScintillaWin::GetScrollInfo(int nBar, LPSCROLLINFO lpsi) {
        // [MJR]
        if (nBar == SB_VERT && vscroll != 0)
                return ::GetScrollInfo(vscroll, SB_CTL, lpsi) ? true : false;
        else if (nBar == SB_HORZ && hscroll != 0)
                return ::GetScrollInfo(hscroll, SB_CTL, lpsi) ? true : false;
        else
...
// begin [MJR]
PRectangle ScintillaWin::GetClientRectangle() {
        PRectangle rc;
        rc = ScintillaBase::GetClientRectangle();

        // adjust for the discrete scrollbar controls, if present
        if (vscroll != 0)
                rc.right -= ::GetSystemMetrics(SM_CXVSCROLL);
        if (hscroll != 0)
                rc.bottom -= ::GetSystemMetrics(SM_CYHSCROLL);
...
STDMETHODIMP ScintillaWin::DragEnter(LPDATAOBJECT pIDataSource, DWORD grfKeyState,
                                     POINTL ptl/*MJR*/, PDWORD pdwEffect) {
        ...
        if (dthelper != 0)
        {
                ...
                dthelper->DragEnter(MainHWND(), pIDataSource, &pt, *pdwEffect);
        }
        // [mjr] end
...
//                if (pIDataSource->QueryGetData(&fmte) == S_OK)
//                {
//                        hasOKFile = true;
...
STDMETHODIMP ScintillaWin::DragOver(DWORD grfKeyState, POINTL pt, PDWORD pdwEffect) {
        try {
                ...
                {
                        POINT ptp = { pt.x, pt.y };
                        dthelper->DragOver(&ptp, *pdwEffect);
                }
//                // [mjr] end
//                if (hasOKFile) {
//                        *pdwEffect = DROPEFFECT_COPY;
...
STDMETHODIMP ScintillaWin::Drop(LPDATAOBJECT pIDataSource, DWORD grfKeyState,
        ...
        try {
                ...
                {
                        POINT ptp = { pt.x, pt.y };
                        dthelper->Drop(pIDataSource, &ptp, *pdwEffect);
                }
                // [mjr] end
...
//                // [mjr] begin
//                if (hasOKFile) {
//                        FORMATETC fmtu = { CF_HDROP, NULL, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
//                        if (pIDataSource->GetData(&fmtu, &medium) == S_OK)
//                        {
//                                *pdwEffect = DROPEFFECT_COPY;
...
// begin [mjr]
// Get the "true" parent window - this is the nearest parent that's a
// popup or MDI child window
HWND ScintillaWin::GetTrueParent() {
        HWND w, wlast;

    // walk up the parent list until we find a popup or MDI child
        for (w = wlast = MainHWND() ; w != 0 ; wlast = w, w = ::GetParent(w))
        {
                if ((GetWindowLong(w, GWL_STYLE) & WS_POPUP) != 0
                    || (GetWindowLong(w, GWL_EXSTYLE) & WS_EX_MDICHILD) != 0)
                        return w;
        }

    // didn't find a window, strangely; return the top parent
        return wlast;
}

// is the given window maximized?
BOOL ScintillaWin::IsWindowMaximized(HWND hwnd) {
    // if it's an MDI child, compare to the MDI client; otherwise, it's easy
        if ((GetWindowLong(hwnd, GWL_EXSTYLE) & WS_EX_MDICHILD) != 0) {
                RECT clirc, winrc;
                GetClientRect(GetParent(hwnd), &clirc);
                GetClientRect(hwnd, &winrc);
                return winrc.right == clirc.right && winrc.bottom == clirc.bottom;
        } else {
