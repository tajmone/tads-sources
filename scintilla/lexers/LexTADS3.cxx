...
/*
 *   TADS3 is a language designed by Michael J. Roberts for the writing of
 *   text based games.  TADS comes from Text Adventure Development System.
 *   It has good support for the processing and outputting of formatted text
 *   and much of a TADS program listing consists of strings.
 *
 *   TADS has two types of strings, those enclosed in single quotes (') and
 *   those enclosed in double quotes (").  These strings have different
 *   symantics and can be given different highlighting if desired.
 *
 *   Both single-quoted and double-quoted strings can be *triple quoted* as
 *   well.  This means that the opening and closing quote marks are tripled.
 *   Individual (or paired) marks within the string are just ordinary
 *   characters.  For example, """This string contains "nested quotes"!""".
 *   Triple-quoted strings have two non-obvious rules.  First, they're
 *   greedy: if there's a run of N>3 close quotes within a triple-quoted
 *   string, the *last* three quotes form the closing delimiter, and the
 *   preceding N-3 quotes are part of the string.  For example,
 *   '''''''''''''''' (16 single quotes) is interpreted as a string with
 *   contents '''''''''' (10 single quotes), because the first three quotes
 *   are the opening delimiter, the last three are the closing delimiter, and
 *   the remaining 10 are the contents.  Second, a backslash before a quote
 *   mark also escapes all consecutive quotes that follow.  For example,
 *   '''\'''''hello'''''''' contains the text '''''hello'''''.  The entire
 *   run of five quotes after the \ are escaped, so they're part of the
 *   string and do not count as a delimiter.  The eight that follow 'hello'
 *   are unescaped, so as per the first rule, the first five of those are
 *   part of the string's contents and the last three are the closing
 *   delimiter.
 *
 *   There can be embedded within both types of strings html tags ( <tag
 *   key=value> ), library directives ( <.directive> ), and message
 ...
 *   may themselves contain single or double quoted strings, although the
 *   double quoted strings may not contain interpolated expressions.
 *
...
/*
 *   Line state flags.  These keep track of additional context for styles
 *   that we overload:
 *
 *   Within a << >> expression, we show most tokens as their ordinary types.
 *   This means that we have to set a side flag that lets us know we're in
 *   one of these experssions to start with.
 *
 *   A << >> expression can be used within an HTML tag, so we need to
 *   remember this as well.
 *
 *   X_STRING is used for single- and double-quoted strings within << >>
 *   expressions, so we need to keep track of which kind we're in.
 *
 *   The T3_TRIPLE_QUOTE flag indicates when we're in a triple-quoted string.
 *   Note that the meaning is orthogonal to T3_SINGLE_QUOTE.  The latter
 *   tells us the *character* used to quote the string, ['] or ["].  The
 *   former tells us *how many copies* of that character are needed to
 *   terminate the string - 1 if not set, 3 if set.
 */
static const int T3_SINGLE_QUOTE = 1;    /* current string is single-quoted */
static const int T3_TRIPLE_QUOTE = 2;   /* current string is triple-quoted */
static const int T3_INT_EXPRESSION = 4;    /* in an interpolated expression */
static const int T3_INT_EXPRESSION_IN_TAG = 8;           /* ...within a tag */
static const int T3_INT_EXPRESSION_IN_QATTR = 16;  /* ...within quoted attr */
static const int T3_HTML_SQUOTE = 32;     /* ...within a single-quoted attr */

static const int T3_OUTER_SQ = 0x10000;  /* outer string uses single quotes */
static const int T3_OUTER_TQ = 0x20000;    /* outer string is triple-quoted */

/*
 *   We use the line state as a primitive stack, with one level of depth.
 *   When we're in an embedded expression, we need to keep two bits of
 *   information about our container string: whether it's a single or double
 *   quote string, and whether it's an ordinary string or a triple-quoted
 *   string.  push_line_state() copies these bits into the "enclosing" part
 *   of the line state flags, and pop_line_state() pulls them back out and
 *   establishes the correct enclosing state.
 */
static void push_line_state(int &lineState, int chQuote)
{
    lineState &= ~(T3_OUTER_SQ | T3_OUTER_TQ);
    if (chQuote == '\'')
        lineState |= T3_OUTER_SQ;
    if (lineState & T3_TRIPLE_QUOTE)
        lineState |= T3_OUTER_TQ;
}

static void pop_line_state(int &lineState, int &quoteCh)
{
    lineState &= ~(T3_SINGLE_QUOTE | T3_TRIPLE_QUOTE);
    quoteCh = '"';
    if (lineState & T3_OUTER_SQ)
    {
        lineState |= T3_SINGLE_QUOTE;
        quoteCh = '\'';
    }
    if (lineState & T3_OUTER_TQ)
        lineState |= T3_TRIPLE_QUOTE;
}
...
static void ColouriseTADSHTMLString(StyleContext &sc, int &lineState) {
    ...
    if (endState == SCE_T3_HTML_STRING) {
        ...
        if (lineState & T3_INT_EXPRESSION) {
    ...
    while (sc.More()) {
        ...
        if (sc.Match('<', '<')) {
            push_line_state(lineState, chString);
            lineState |= T3_INT_EXPRESSION | T3_INT_EXPRESSION_IN_QATTR;
...
static void ColouriseTADS3HTMLTag(StyleContext &sc, int &lineState) {
    ...
    while (sc.More()) {
        ...
        if (sc.Match('<', '<')) {
            push_line_state(lineState, chQuote);
...
static void ColouriseTADS3String(StyleContext &sc, int &lineState) {
    ...
    const char tquote[] = { (char)chQuote, (char)chQuote, (char)chQuote, 0 };
    while (sc.More()) {
        if (IsEOL(sc.ch, sc.chNext)) {
            return;
        }
        if (sc.ch == chQuote) {
            if (lineState & T3_TRIPLE_QUOTE)
            {
                /* triple-quote - we need three copies */
                if (sc.Match(tquote))
                {
                    /*
                     *   Triple-quotes are greedy: if we have N>3 of them in
                     *   a row, N-3 are part of the string.  Skip ahead until
                     *   we no longer have three more to skip.
                     */
                    for (sc.Forward() ; sc.Match(tquote) ; sc.Forward()) ;

                    /* skip the last two and switch to the end state */
                    sc.Forward();
                    sc.ForwardSetState(endState);

                    /* clear the single and triple-quote flags */
                    lineState &= ~T3_TRIPLE_QUOTE;
                    if (chQuote == '\'')
                        lineState &= ~T3_SINGLE_QUOTE;
            ...
            {
                /* ordinary string - we just need one quote to end it */
                sc.ForwardSetState(endState);
                if (chQuote == '\'')
                    lineState &= ~T3_SINGLE_QUOTE;
                return;
...
//[mjr]     if (sc.state == SCE_T3_D_STRING && sc.Match('<', '<')) {
        if (sc.Match('<', '<')) { //mjr
            push_line_state(lineState, chQuote);
        ...
        if (sc.Match('\\', '<')) {
            /* quoted < - this suppresses an embedding, but not html tags */
            sc.Forward();
            if (sc.Match('<', '<'))
                sc.Forward();
        } else if (sc.Match('\\', static_cast<char>(chQuote))
                   || sc.Match('\\', '\\')) {

            /*
             *   in a triple-quoted string, \ escapes a run of consecutive
             *   quote characters
             */
            sc.Forward();
            if ((lineState & T3_TRIPLE_QUOTE) && sc.ch == chQuote)
            {
                /* skip all consecutive quotes */
                for (sc.Forward() ; sc.ch == chQuote ; sc.Forward()) ;
            }
            else
            {
                /* in a regular string, just skip the quote or backslash */
                sc.Forward();
            }
        } else if (sc.ch == '{') {
        ...
        switch(sc.state) {
            ...
            case SCE_T3_X_DEFAULT:
                           ...
                           && sc.Match('>', '>'))
                {
                    sc.Forward(2);
                    int chQuote;
                    pop_line_state(lineState, chQuote);
                    sc.SetState(lineState & T3_SINGLE_QUOTE // mjr
                        ? SCE_T3_S_STRING : SCE_T3_D_STRING); // mjr
                    if (lineState & T3_INT_EXPRESSION_IN_QATTR)
                        sc.SetState(SCE_T3_HTML_STRING);
                    if (lineState & T3_INT_EXPRESSION_IN_TAG)
                        sc.SetState(SCE_T3_HTML_DEFAULT);
                    lineState &= ~(
                        T3_INT_EXPRESSION
                        | T3_INT_EXPRESSION_IN_QATTR
