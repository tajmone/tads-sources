...
void CHtmlFormatter::run_table_pass_two(CHtmlTagTABLE *table_tag)
{
     ...
     *
     *   The behavior of tag formatting on this pass will be largely the same
     *   as it was on the first pass, except that the table tags will see
     *   that they're in pass two, and so will format their contents at the
     *   proper sizes.  So, the results of this second pass will be directly
     *   usable for displaying.  In addition, since we're in pass two, we
     *   won't try to run pass two again when we finish with the table.
     *
     *   Note that we want to format tags until we get to the present
     *   curtag_, and then format that tag as well - the enclosing tag
     *   scanning loop will have last set curtag_ to the final tag within the
     *   table, and clearly we need to format that tag again before we
     *   finish.
     *
     *   However, there's one special case to worry about.  If the table was
     *   unclosed on the last formatting round, and has just been closed,
     *   we'll format the table as a side effect of checking for more tags
     *   (via more_to_do()), and in this case curtag_ will still be null from
     *   the last (partial) formatting round.  So if we waited to reach
     *   curtag_, we'd wait forever and blast through the whole rest of the
     *   formatting list - which we'd then do *again* as we proceed on from
     *   the table.  So, we need to stop if we reach a tag outside of the
     *   table.  To check for this case, note the tag that comes after the
     *   table in formatting order, and stop if we encounter this tag; don't
     *   format this tag, since it lies outside of the table and will be
     *   formatted separately when we get back to the main loop.
     */
    post_table_tag = table_tag->get_post_table_tag();
    for (tag = table_tag ; tag != 0 ; )
    {
        ...
        /*
         *   if the new tag is the post-table tag, we've blasted out of the
         *   table, so we want to stop here
         */
        if (nxt == post_table_tag)
            break;
                                ...
                                HTML_Attrib_id_t sequence)
{
    ...
    /* play the sound */
    queue->add_sound(
        res, repeat_count, random_start, sequence,
        tag->get_volume(), tag->get_fade_in(), tag->get_fade_out(),
        tag->get_crossfade_in(), tag->get_crossfade_out());
}

/*
 *   Cancel the current sound in a given layer
 */
void CHtmlFormatter::cancel_sound(HTML_Attrib_id_t layer, double fade_out,
                                  int bg_fade, int sync)
{
    /* check the layer */
    switch(layer)
    {
    case HTML_Attrib_background:
        if (sound_queues_[0] != 0)
            sound_queues_[0]->stop_current_sound(sync, fade_out, bg_fade);
        break;

    case HTML_Attrib_bgambient:
        if (sound_queues_[1] != 0)
            sound_queues_[1]->stop_current_sound(sync, fade_out, bg_fade);
        break;

    case HTML_Attrib_ambient:
        if (sound_queues_[2] != 0)
            sound_queues_[2]->stop_current_sound(sync, fade_out, bg_fade);
        break;

    case HTML_Attrib_foreground:
        if (sound_queues_[3] != 0)
            sound_queues_[3]->stop_current_sound(sync, fade_out, bg_fade);
        break;
    ...
    default:
        /* cancel sound in all layers */
        {
            ...
            for (i = 0 ; i < 4 ; ++i)
            {
                ...
                {
                    sound_queues_[i]->stop_current_sound(
                        sync, fade_out, bg_fade);
                }
            }
        }
        break;
...
void CHtmlFormatterSrcwin::start_at_top(int reset_sounds)
{
    ...
    /*
     *   restore the font - ignore the window default, since we want to use
     *   the explicit font that the debugger told us to use
     */
    curfont_ = font;
}
                 ...
                 CHtmlTagBANNER *banner_tag)
{
    ...
    subwin->set_html_input_color(0, TRUE);
    ...
    unsigned long style)
{
    ...
    win->set_html_input_color(0, TRUE);
...
void CHtmlFormatterBannerExtGrid::get_grid_font(int keep_color)
{
    ...
    /* get the font, if we have a window available */
    if (get_win() != 0)
