...
Notes
  This file is NOT required for the portable system.  This is a helper file
  that ports can optionally use.  You should only use this file if you want
  to use the libpng portable reference library to implement PNG image
  handling.  If you use libpng, this file is helpful because it implements a
  simple, portable wrapper for libpng that makes it easier to use within the
  HTML TADS type system.  Some GUI operating systems have their own native
  support for PNG image files, so some ports may prefer not to use libpng at
  all, but to use the native OS APIs for PNGs instead.  If you're using
  native OS PNG handling, or you're using some other third-party library
  instead of libpng, you will have no use for this file and should simply
  omit it from your makefile.
Modified
