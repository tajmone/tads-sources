...
Notes
  ...
  NOTE: This file is NOT required for the portable system.  This is
  a helper file that ports can optionally use.  You should only use
  this file if you want to use the IJG portable reference library to
  implement JPEG image handling.  If you use the IJG library, this
  file is helpful because it implements a simple, portable wrapper
  for the IJG library that makes it easier to use within the HTML
  TADS type system.  Many GUI operating systems have their own native
  support for JPEG image files, so some ports may prefer not to use
  the IJG library at all, but to use the native OS APIs for JPEGs
  instead.  If you're using native OS JPEG handling, or you're using
  some other third-party library instead of the IJG library, you will
  have no use for this file and should simply omit it from your
  makefile.
Modified
