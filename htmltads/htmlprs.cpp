...
#include "htmlreg.h"
    ...
    static struct attr_t attr_list[] =
    {
        ...
        { "input", HTML_Attrib_input },
        { "volume", HTML_Attrib_volume },
    ...
    /* create the special outermost container */
    container_depth_ = 1;
...
void CHtmlParser::parse_newline()
{
    ...
    if (obey_whitespace_)
    {
        ...
        /*
         *   add a newline to the text stream, so that we find whitespace
         *   between lines if we perform a search or copy/paste
         */
        get_text_array()->append_text_noref("\n", 1);
...
void CHtmlParser::parse_text()
{
    ...
    if (special != 0)
    {
        ...
        add_space_no_eat:
            /* add the space tag */
            append_tag(new CHtmlTagSpace(
                text_array_, spwid, obey_whitespace_ && !break_long_lines_));
 ...
 *   processed so far.
 */
void CHtmlParser::parse(const textchar_t *src, size_t len,
                        CHtmlSysWinGroup *frame)
{
