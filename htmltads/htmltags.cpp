                                      ...
                                      const textchar_t *val, size_t vallen)
{
    ...
    else
    {
        ...
        case HTML_Attrib_input:
            *color_var = HTML_COLOR_INPUT;
            break;
...
/*
 *   Set a numeric attribute from a floating point value.
 */
HTML_attrerr CHtmlTag::set_float_attr(double *num_var,
                                      const textchar_t *val, size_t vallen)
{
    double acc;
    int in_frac = FALSE;
    ...
    /* loop through the digits */
    for (err = HTML_attrerr_ok, acc = 0.0 ; vallen ; ++val, --vallen)
    {
        if (is_digit(*val))
        {
            /* accumulate one more digit */
            if (in_frac)
            {
                acc += (double)ascii_digit_to_int(*val) / frac_base;
                frac_base *= 10.0;
            }
            else
            {
                acc *= 10.0;
                acc += (double)ascii_digit_to_int(*val);
            }
        }
        else if (*val == '.' && !in_frac)
        {
...
/*
 *   Check an attribute string for a comma-delimited keyword
 */
int CHtmlTag::check_comma_kw(const textchar_t **val, size_t *vallen,
                             const textchar_t *kw)
{
    ...
    if (*vallen >= kwlen
        && memicmp(*val, kw, kwlen) == 0
        && (*vallen == kwlen || (*val)[kwlen] == ','))
    {
        /* we found the keyword - skip it in the input */
        *val += kwlen;
        *vallen -= kwlen;
 ...
 *   get the default indenting
 */
long CHtmlTagOL::get_left_indent(CHtmlSysWin *win,
                                 CHtmlFormatter *formatter) const
{
    const textchar_t *prefix;

    /* base our indent on our numbering style */
    switch (style_)
    {
    case '1':
    default:
    ...
    /* make enough space for a 2-digit prefix */
    return win->measure_text(formatter->get_font(),
                             prefix, get_strlen(prefix), 0).x * 2;
}
                                         ...
                                         const textchar_t *val, size_t vallen)
{
    ...
    case HTML_Attrib_input:
        /* parse the color */
        err = set_color_attr(parser, &inputcolor_, val, vallen);

        /* if we got a valid color, note that we want to use it */
        if (err == HTML_attrerr_ok)
            use_inputcolor_ = TRUE;
...
void CHtmlTagBODY::format(CHtmlSysWin *win, CHtmlFormatter *formatter)
{
    ...
    /* tell the window to use my input color, if one is set */
    win->set_html_input_color(map_color(win, inputcolor_), !use_inputcolor_);

    /* tell the window what to do with the link colors */
    win->set_html_link_colors(
        map_color(win, link_color_), !use_link_color_,
...
/*
 *   get the next tag after the end of the table
 */
CHtmlTag *CHtmlTagTABLE::get_post_table_tag()
{
    CHtmlTagContainer *cont;

    /*
     *   starting with the TABLE tag itself, then working up through our
     *   containers, find the next sibling
     */
    for (cont = this ; cont != 0 && cont->get_next_tag() == 0 ;
         cont = cont->get_container()) ;

    /* return the sibling we found (if any) */
    return (cont != 0 ? cont->get_next_tag() : 0);
}
                                          ...
                                          size_t vallen)
{
    ...
    case HTML_Attrib_fadein:
        /* check for the CROSSFADE qualifier */
        crossfade_in_ = check_comma_kw(&val, &vallen, "crossfade");

        /* get the fade time */
        return set_float_attr(&fadein_, val, vallen);

    case HTML_Attrib_fadeout:
        /* check for the CROSSFADE qualifier */
        crossfade_out_ = check_comma_kw(&val, &vallen, "crossfade");

        /* get the fade time */
        return set_float_attr(&fadeout_, val, vallen);

    case HTML_Attrib_volume:
        /* get the volume */
        ret = set_number_attr(&vol_, val, vallen);

        /* peg it to the 0..100 range */
        if (vol_ < 0)
            vol_ = 0;
        if (vol_ > 100)
            vol_ = 100;

        /* return the result from the attribute parse */
        return ret;
...
void CHtmlTagSOUND::format(CHtmlSysWin *win, CHtmlFormatter *formatter)
{
    ...
    if (cancel_)
    {
        formatter->cancel_sound(layer_, fadeout_, crossfade_out_, FALSE);
    ...
    {
        /*
         *   Cancel the current sound in this layer.  If the
         *   FADEIN=CROSSFADE,time option was set, do a cross-fade: do a
         *   background fade on the old sound with the same duration as the
         *   fade-in on the new sound.
         */
        formatter->cancel_sound(
            layer_, crossfade_in_ ? fadein_ : 0.0, crossfade_in_, FALSE);
