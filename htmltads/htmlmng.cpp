...
Notes
  ...
  to use the mnglib portable reference library to implement MNG image
  handling.  If you use mnglib, this file is helpful because it implements a
  simple, portable wrapper for mnglib that makes it easier to use within the
  HTML TADS type system.  Some GUI operating systems have their own native
  support for MNG image files, so some ports may prefer not to use mnglib at
  all, but to use the native OS APIs for MNGs instead.  If you're using
  native OS MNG handling, or you're using some other third-party library
  instead of mnglib, you will have no use for this file and should simply
