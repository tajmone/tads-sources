...
public:
    ...
    virtual void active_to_front()
    {
        /*
         *   we're always embedded in a docking parent, so we need to bring
         *   our parent window to the top
         */
...
public:
    ...
    HRESULT STDMETHODCALLTYPE Drop(IDataObject __RPC_FAR *pDataObj,
                                   ...
                                   DWORD __RPC_FAR *pdwEffect);
protected:
    /* do we save our expression list in the configuration? */
    virtual int config_saves_expr_list() { return TRUE; }
    ...
    /* fixed horizontal scrolling width */
    static const int SCROLL_WIDTH = 1000;
    ...
    void draw_focus(int draw, HDC hdc);
    ...
    /* flag: we're tracking a valid drag/drop */
    int drop_ok_ : 1;
...
public:
    ...
    /*
     *   we don't save our expression list for locals, since it's dynamically
     *   generated from the run-time state
     */
    virtual int config_saves_expr_list() { return FALSE; }
...
public:
    ...
    const textchar_t *get_parent_rel() const
        { return parent_relationship_.get(); }
