    ...
    : CHtmlSysWin(formatter), CTadsWinScroll(has_vscroll, has_hscroll)
{
    ...
    /* use off-screen rendering to reduce flicker */
    ...
    /* use the default Windows selection colors */
    default_sel_fg_ = TRUE;
    default_sel_bg_ = TRUE;
    ...
    /* set it to use the default character set */
    cmdbuf_->set_charset(owner->owner_get_default_charset());
...
void CHtmlSysWin_win32::set_status_line(CTadsStatusline *statusline)
{
    ...
    statusline->main_part()->register_source(this);
}
             ...
             && memicmp(nm_input, name, len) == 0)
    {
        ...
        *base_point_size = prefs_->get_inpfont_size();
        ...
        if (tlf->face_set_explicitly)
        {
            ...
            font_desc->color = HTML_COLOR_INPUT;
...
CHtmlSysFont *CHtmlSysWin_win32::get_font(const CHtmlFontDesc *in_font_desc)
{
    ...
    tlf.codepage = font_desc.charset.codepage;
 ...
 */
TadsCmdStat_t CHtmlSysWin_win32::check_command(const check_cmd_info *info)
{
    int paused;
    ...
    case ID_HELP_COMMAND:
         ...
         */
        return is_cmd_evt_enabled(cmd_id_to_os_cmd_id(info->command_id))
    ...
    default:
        ...
        return CTadsWinScroll::check_command(info);
    }
}
                                  ...
                                  int command_id, HWND ctl)
{
    ...
    /* check the command to make sure that it's really enabled */
    if (!check_command_enabled(command_id))
        return TRUE;
 ...
 *   exeucute a search
 */
int CHtmlSysWin_win32::execute_find(
    const textchar_t *findstr, int exact_case,
    int start_at_top, int wrap, int dir, int whole_word,
    unsigned long first_match_a, unsigned long first_match_b)
{
    ...
    for (;;)
    {
        ...
        found = get_formatter()->search_for_text(
            findstr, get_strlen(findstr), exact_case, whole_word,
            wrap, dir, start_ofs, &sel_start, &sel_end);

        /*
         *   if we didn't find it, or we wrapped back to the first match,
         *   display a message and stop looking
         */
        if (!found
    ...
    /* return found/not found indication */
    return found;
}
 ...
 *   Process an ALT key
 */
int CHtmlSysWin_win32::do_syschar(TCHAR c, unsigned long /*keydata*/)
{
    /* check the key */
    switch(c)
    {
...
int CHtmlSysWin_win32::do_user_message(int msg, WPARAM wpar, LPARAM lpar)
{
    ...
    case HTMLM_SOUND_DONE:
        /* call the sound object and tell it that playback is done */
        ((CTadsAudioPlayer *)lpar)->on_sound_done((int)wpar);
        return TRUE;
 ...
 */
unsigned int CHtmlSysWin_win32::get_default_charset()
{
    /* get the default character set ID from the owner window */
    return get_owner()->owner_get_default_charset().charset;
}

/*
 *   Apply the given profile to the current game
 */
void CHtmlSysWin_win32::set_game_specific_profile(const char *profile)
{
    /* send this request to the main window */
    CHtmlSys_mainwin::get_main_win()->set_game_specific_profile(profile);
}
                                           ...
                                           CHtmlSysWin_win32 *first_chi)
{
    ...
    if (is_banner_win_)
    {
        ...
        case HTML_BANNERWIN_UNITS_PCT:
             ...
             *   parent space
             */
            wid = (stored_wid * (parent_rc->right - parent_rc->left)) / 100;
            break;
        }
        ...
        case HTML_BANNERWIN_UNITS_PCT:
             ...
             *   parent size
             */
            ht = (stored_ht * (parent_rc->bottom - parent_rc->top)) / 100;
            break;
        }
        ...
        case HTML_BANNERWIN_POS_TOP:
            ...
            if (has_border_)
            {
                ...
                SetRect(&border_rc, new_rc.left, new_rc.bottom,
                        new_rc.right - new_rc.left, 1);
            }
        ...
        case HTML_BANNERWIN_POS_BOTTOM:
            ...
            if (has_border_)
            {
                ...
                SetRect(&border_rc, new_rc.left, new_rc.top - 1,
                        new_rc.right - new_rc.left, 1);
            }
        ...
        case HTML_BANNERWIN_POS_LEFT:
            ...
            if (has_border_)
            {
                ...
                SetRect(&border_rc, new_rc.right, new_rc.top,
                        1, new_rc.bottom - new_rc.top);
            }
        ...
        case HTML_BANNERWIN_POS_RIGHT:
            ...
            if (has_border_)
            {
                ...
                SetRect(&border_rc, new_rc.left - 1, new_rc.top,
                        1, new_rc.bottom - new_rc.top);
            }
...
textchar_t *CHtmlSysWin_win32::get_status_message(int *caller_deletes)
{
    ...
    else if (formatting_msg_)
    {
        ...
        msg = working_str_.get();
             ...
             && (((msg = status_link_->title_.get()) != 0
                  && msg[0] != '\0')
                 || (((msg = (textchar_t *)status_link_->href_.get_url()) != 0)
                     && msg[0] != '\0')))
    {
        /*
         *   We're hovering over a link, and the link has a non-empty title
         *   OR a non-empty URL - use what we found.  Note that the logic of
         *   the condition above will skip this and continue on to other
         *   lower-priority options if we're not hovering over a link, OR we
         *   are hovering and the title & URL are both undefined or empty.
         *   In those cases, the link doesn't seem to want to display
         *   anything for its status text, so we'll act as though we don't
         *   have a hover link at all and continue on to other status-line
         *   message sources.
         */
    }
 ...
 */
void CHtmlSysWin_win32::do_paint_content(HDC dc, const RECT *rc)
{
    CHtmlRect area;
    RECT more_rc;
    int clip_lines;
                                       ...
                                       const RECT *doc_cliprect)
{
    ...
    if (hilite)
    {
        /* set highlight text color and background color */
    ...
    else
    {
        ...
        /*
         *   If the font specifies a color, use the font color, otherwise use
         *   the default text color.  Note that HTML_COLOR_INPUT is special -
         *   we use this color internally to indicate a <FONT FACE=INPUT>
         *   font, which gets its color dynamically from the current input
         *   color setting at the time of drawing.
         */
...
void CHtmlSysWin_win32::fmt_adjust_vscroll()
{
    ...
    /* assume we won't switch to more mode */
    int entered_more_mode = FALSE;
    ...
    /*
     *   If we entered more mode, check to see if we're already scrolled to
     *   the bottom of the window, and cancel more mode if so.  Because our
     *   scrolling increment is the font line height, our preliminary
     *   decision about the new scrolling position is sometimes off due to
     *   rounding.  When the final position is at the bottom of the scroll
     *   range, there's no need for more mode.
     */
    if (entered_more_mode)
    {
        /* retrieve the actual new scroll position */
        SCROLLINFO vsinfo;
...
void CHtmlSysWin_win32::inval_doc_coords(const CHtmlRect *doc_area)
{
    ...
    /*
     *   Set the system rectangle - use a one-pixel overhang at the right and
     *   bottom edges to ensure that we invalidate the entire area if the
     *   input rectangle is inclusive of its endpoints.  Further inflate the
     *   whole area by one pixel in each direction - on Windows, the OS-level
     *   anti-aliasing mechanism can expand the actual drawing area of text
     *   by a pixel outside of its nominal bounding box (that is, the OS
     *   reports the bounding box as too small when asked to measure text).
     */
    SetRect(&rc, screen_area.left - 1, screen_area.top - 1,
            screen_area.right + 2, screen_area.bottom + 2);
...
void CHtmlSysWin_win32::notify_sound_pref_change()
{
    ...
    {
        formatter_->cancel_sound(HTML_Attrib_ambient, 0.0, FALSE, FALSE);
        formatter_->cancel_sound(HTML_Attrib_bgambient, 0.0, FALSE, FALSE);
        formatter_->cancel_sound(HTML_Attrib_foreground, 0.0, FALSE, FALSE);
    }

    /* if music is now turned off, cancel playback in the music layer */
    if (!prefs_->get_music_on())
        formatter_->cancel_sound(HTML_Attrib_background, 0.0, FALSE, FALSE);
}
    ...
    int use_windows_sel_colors, int use_windows_sel_bgcolor)
{
    ...
    set_selected_text_color(use_windows_sel_colors, sel_text_color,
                            use_windows_sel_bgcolor, sel_bkg_color);
}
...
/*
 *   Set the default input text color
 */
void CHtmlSysWin_win32::set_html_input_color(HTML_color_t color,
                                             int use_default)
{
    COLORREF new_color;
    ...
    if (use_default || prefs_->get_override_colors())
    {
        ...
        new_color = HTML_color_to_COLORREF(prefs_->get_inpfont_color());
    }
    else
    {
        /* use the color they provided */
    ...
    /* set the color */
    internal_set_input_color(new_color);
}

/*
 *   Internal routine to set the input text color
 */
 ...
 */
TadsCmdStat_t CHtmlSysWin_win32_Hist::check_command(
...
textchar_t *CHtmlSysWin_win32_Input::get_status_message(int *caller_deletes)
{
    ...
    textchar_t *msg = CHtmlSysWin_win32::get_status_message(caller_deletes);
    if (msg != 0)
        return msg;
...
/*
 *   Check for an EOF input loop.  When the user manually closes the main
 *   window, and the game is still running, we don't just kill the bytecode
 *   program.  Instead, we set an internal flag to indicate that there's no
 *   more keyboard/mouse UI, and let the program continue so that it can shut
 *   itself down gracefully.  The program finds out that the UI has been
 *   closed by receiving an EOF error code from any keyboard or event input
 *   function.  Well-behaved programs should finish up any business and then
 *   terminate on seeing EOF.
 *
 *   Some older games ignore the EOF error.  In most cases such games will
 *   think they got a valid key/event, so they'll do some processing and go
 *   back for another key/event.  Thus they get stuck in an input loop: we'll
 *   immediately answer each input request with an EOF indication, and
 *   they'll immediately come back for another input.  The program will spin
 *   at 100% CPU, which is especially pernicious since it doesn't have a
 *   window to let the user see that it's still running.  The machine will
 *   just mysteriously bog down until the user kills the process with Task
 *   Manager.
 *
 *   This is a simple watchdog to catch this kind of errant program and kill
 *   it automatically, without the need for user intervention.  Whenever the
 *   program tries to read an event, and our internal EOF flag is set, we'll
 *   increment a counter.  If the counter gets too high, we'll assume the
 *   program is looping, and kill it.
 *
 *   Note that some games have legitimate reasons to keep running after the
 *   main window has been closed, such as to save data, close files, and
 *   otherwise clean up before terminating.  However, a program that's doing
 *   something legitimate won't loop on input; it shouldn't be asking for
 *   input at all, really.
 */
void CHtmlSysWin_win32_Input::check_eof_input_loop()
{
    /*
     *   Ignore this check if we're running in the debugger.  In this case,
     *   we want to leave the program running so that the user can take
     *   control through the debugger UI.
     */
    CHtmlSys_mainwin *mainwin = CHtmlSys_mainwin::get_main_win();
    if (mainwin && mainwin->is_in_debugger())
        return;

    /*
     *   count the post-EOF input request; if this takes us over our
     *   (arbitrary) limit, assume we're looping and kill the program
     */
    if (++eof_input_counter_ > 10000)
    ...
    textchar_t *ch, int pause_only, time_t timeout, int use_timeout)
{
    ...
    if (last_keystroke_cmd_ != 0)
    {
        ...
        *ch = (textchar_t)last_keystroke_cmd_;
        ...
        /*
         *   check for EOF - this means that the user manually closed the
         *   main window, so the bytecode program is now running without a
         *   UI, thus "end of file" on the keyboard input
         */
        if (eof_)
        {
            /* check to see if the program is looping on input requests */
            check_eof_input_loop();
...
void CHtmlSysWin_win32_Input::get_input_begin(size_t bufsiz)
{
    ...
    /* set the input buffer character set to the current font */
    CHtmlSysFont *font = formatter_->get_font();
    if (font != 0)
        cmdbuf_->set_charset(font->get_charset());
 ...
 */
int CHtmlSysWin_win32_Input::do_syschar(TCHAR c, unsigned long keydata)
{
                                      ...
                                      DWORD __RPC_FAR *effect)
{
    ...
    DropHelper_Enter(dataobj, grfKeyState, pt, effect);
...
{
    /* let the helper object do its work, if any */
    DropHelper_Over(grfKeyState, pt, effect);
    ...
    CHtmlPoint cmdloc = doc_to_screen(
        formatter_->get_text_pos(cmdtag_->get_text_ofs()));
...
{
    /* let the helper object do its work, if any */
    DropHelper_Drop(dataobj, grfKeyState, pt, effect);
                                   ...
                                   CHtmlParser *parser, int in_debugger)
{
    ...
    /*
     *   Find the character set that corresponds to the current system ANSI
    ...
    /*
     *   Get the default character in the default character set -- we'll use
     *   this whenever the game tries to display a character for which we can
     *   provide no character mapping.  Use the DC for the desktop window,
     *   which presumably has a font selected that uses the system default
     *   character set.
     */
...
void CHtmlSys_mainwin::end_current_game()
{
    ...
    /* cancel sounds */
    main_panel_->get_formatter()->cancel_sound(
        HTML_Attrib_invalid, 0.0, FALSE, FALSE);
...
void CHtmlSys_mainwin::set_recent_game_menu()
{
    ...
    if (found)
    {
        ...
        if (standalone_exe_)
        {
            ...
            /* look to see if we have a Load Game toolbar button */
            TBBUTTONINFO tbinfo;
            tbinfo.cbSize = sizeof(tbinfo);
            tbinfo.dwMask = TBIF_COMMAND;
            int idx = SendMessage(toolbar_, TB_GETBUTTONINFO,
                                  ID_FILE_LOADGAME, (LPARAM)&tbinfo);

            if (idx != -1 && tbinfo.idCommand == ID_FILE_LOADGAME)
                SendMessage(toolbar_, TB_DELETEBUTTON, idx, 0);
        }
    }
...
int CHtmlSys_mainwin::do_close()
{
    ...
    allow_close:
        ...
        main_panel_->get_formatter()->cancel_sound(
            HTML_Attrib_invalid, 0.0, FALSE, TRUE);
        main_panel_->get_formatter()->cancel_playback();
...
int CHtmlSys_mainwin::do_user_message(int msg, WPARAM, LPARAM)
{
    ...
    else
    {
        switch (msg)
        {
        case HTMLM_BRING_TO_FRONT:
            /* bring the window to the front */
            bring_owner_to_front();

            /* if there's a web UI window, put it in front of me */
                                 ...
                                 int command_id, HWND ctl)
{
    ...
    case ID_VIEW_TOOLBAR:
        ...
        /*
         *   synthesize a resize to recalculate the window layout - adding or
         *   removing the toolbar will change the available area for the html
         *   windows
         */
        synth_resize();
 ...
 */
TadsCmdStat_t CHtmlSys_mainwin::check_command(const check_cmd_info *info)
{
    /* check to see if it's one of our own commands */
    switch(info->command_id)
    {
    ...
    case ID_FILE_LOADGAME:
         ...
         *   running under it
         */
        return standalone_exe_ || in_debugger_ || main_panel_->get_exit_pause()
            ? TADSCMD_DISABLED : TADSCMD_ENABLED;
    ...
    default:
         ...
         */
        if (info->command_id >= ID_PROFILE_FIRST
            && info->command_id <= ID_PROFILE_LAST)
            ...
            && hist_panel_ != 0)
        {
            ...
            return hist_panel_->check_command(info);
        }
        else if (main_panel_ != 0)
        {
            /* send it to the main panel */
            return main_panel_->check_command(info);
        }
        break;
    }
...
void CHtmlSys_mainwin::delete_orphaned_banners()
{
    ...
    /* if there are no orphaned banners, there's nothing to do */
    if (orphaned_banner_cnt_ == 0)
        return;
...
void CHtmlSys_mainwin::display_output(const textchar_t *buf, size_t len)
{
    ...
    /*
     *   On any explicit text output, clear the Web UI mode flag, since we're
     *   using the console window after all.  Don't count control sequences
     *   ("<?...>" codes).  This doesn't mean it's not also a Web UI game,
     *   but it does mean that the console window should no longer be hidden.
     */
    if (webui_game_)
    {
        /* scan past <?...> codes */
        while (len > 2 && *buf == '<' && *(buf+1) == '?')
        {
            /* it's a control code - skip to the matching > */
            for (char qu = 0 ; len > 0 ; ++buf, --len)
            {
                if (qu == 0 && (*buf == '"' || *buf == '\''))
                    qu = *buf;
                else if (qu != 0 && qu == *buf)
        ...
        /*
         *   if there's anything left after skipping ignored sequences, we do
         *   indeed have text output
         */
        if (len != 0)
        {
            /* clear the Web UI flag */
            webui_game_ = FALSE;
...
/*
 *   Get my selection text
 */
int CHtmlSys_dbgwin::get_sel_text(CStringBuf *buf, unsigned int flags,
                                  size_t maxlen)
{
    CHtmlFormatter *fmt;
    ...
    /* get the selection range from my html panel's formatter */
    fmt = ((CHtmlSysWin_win32 *)html_panel_)->get_formatter();
    fmt->get_sel_range(&start, &end);

    /*
     *   If the range is empty, and they want to extend to the nearest word
     *   boundaries, do so; handle "expression" extension simply as word
     *   extension.
     */
    if (start == end && (flags & (DSW_WORD_EXTEND | DSW_EXPR_EXTEND)) != 0)
        fmt->get_word_limits(&start, &end, start);

    /* if it's too long, return an empty range */
    len = fmt->get_chars_in_ofs_range(start, end);
    if (len > maxlen)
    {
        /* clear the buffer and indicate failure */
        buf->clear();
        return FALSE;
    }

    /* get the text */
    fmt->extract_text(buf, start, end);
 ...
 */
int CHtmlSys_dbgwin::set_sel_range(unsigned long start, unsigned long end)
{
    /* set my range */
    ((CHtmlSysWin_win32 *)html_panel_)->get_formatter()
        ->set_sel_range(start, end);

    /* it's valid for this type of window */
    return TRUE;
}
 ...
 *   find text
 */
int CHtmlSys_dbgwin::active_find_text(
    const textchar_t *txt,
    int exact_case, int wrap, int direction, int regex, int whole_word,
    unsigned long first_match_a, unsigned long first_match_b)
{
    /* ask the HTML panel to do the search */
    if (html_panel_ != 0)
        return html_panel_->execute_find(
            txt, exact_case, FALSE, wrap, direction, whole_word,
...
{
    CHtmlSysWin_win32_dbglog *subwin;
    ...
    subwin = (CHtmlSysWin_win32_dbglog *)get_html_panel();
 ...
 */
TadsCmdStat_t CHtmlSys_dbglogwin::active_check_command(
    const check_cmd_info *info)
{
 ...
 *   Check the status of a command
 */
TadsCmdStat_t CHtmlSys_dbglogwin::check_command(const check_cmd_info *info)
{
    switch(info->command_id)
    {
    case ID_FILE_HIDE:
    ...
    default:
        ...
        if (html_panel_ != 0)
            return html_panel_->check_command(info);
        break;
    }
...
/* ------------------------------------------------------------------------ */
/*
 *   CDebugSysWin - implementation of IDebugSysWin on CHtmlSysWin_win32
 */

/*
 *   go to a source position by line number
 */
void CDebugSysWin::idw_show_source_line(int linenum)
{
    CHtmlDisp *disp;

    /* find the line at this position */
    disp = get_formatter()->find_by_debugsrcpos(linenum);
    ...
    if (disp != 0)
    {
        ...
        pos = disp->get_pos();
        scroll_to_doc_coords(&pos);

        /* put the selection at the start of the line */
        get_formatter()->set_sel_range(disp->get_text_ofs(),
                                       disp->get_text_ofs());
    }
}

/*
 *   reformat the window's contents
 */
void CDebugSysWin::idw_reformat(const CHtmlRect *new_scroll_pos)
{
    CHtmlRect old_scroll_pos;

    /* remember the original scrolling position */
    get_scroll_doc_coords(&old_scroll_pos);

    /* do the initial formatting of the window */
    if (!do_formatting(TRUE, TRUE, TRUE))
    {
        /* make sure we update the whole screen */
        CHtmlRect irc(0, 0, HTMLSYSWIN_MAX_RIGHT, HTMLSYSWIN_MAX_BOTTOM);
        inval_doc_coords(&irc);
    }
    ...
    /*
     *   scroll the window to the new position the caller specified, or back
     *   to the original coordinates if the caller didn't specify a new
     *   location
     */
    scroll_to_doc_coords(
        new_scroll_pos != 0 ? new_scroll_pos : &old_scroll_pos);
}

/*
 *   post-load initialization
 */
void CDebugSysWin::idw_post_load_init()
{
    CHtmlRect rc;

    /* update the display and scroll to the top */
    rc.set(0, 0, 0, 0);
    idw_reformat(&rc);
}
 ...
 *   command status
 */
TadsCmdStat_t CHtmlSysWin_win32_dbglog::check_command(
    const check_cmd_info *info)
{
 ...
 */
void CHtmlSysWin_win32_dbglog::show_error_line(int by_mouse)
{
    unsigned long sel_start, sel_end;
        ...
        if (p < txtp + len && *p == '(')
        {
                ...
                && *(p+2) == ' ')
            {
                ...
                *paren = '\0';
                ...
                /*
                 *   if this is by a mouse event, cancel mouse tracking -
                 *   this is important because we'll lose focus
                 */
                if (by_mouse)
                    end_mouse_tracking(HTML_TRACK_LEFT);

                /* display the file, opening it a the default line */
                debugger_ifc_->dbghostifc_open_source(fname, linenum, 0, TRUE);
...
void CHtmlSys_creditswin::build_contents(CHtmlTextBuffer *txtbuf)
{
    ...
    static const char txt3[] =
        ...
        "<i>libjpeg</i><br>"
        "by The Independent JPEG Group<br><br>"
        ...
        "<b>Mr. Roberts's Text Editor</b><br>"
        ...
        "enthusiasm.<br><br><br><br>"

        /* copyright-date-string */
        "<b>Copyright &copy; 1997, 2012 by Michael J. Roberts</b><br>"
    ...
    txtbuf->append(txt2, sizeof(txt2) - 1);
    txtbuf->append(xtra_lib_credits(), strlen(xtra_lib_credits()));
    txtbuf->append(txt3, sizeof(txt3) - 1);
    txtbuf->append(xtra_copyrights(), strlen(xtra_copyrights()));
    txtbuf->append(txt4, sizeof(txt4) - 1);
}
