    ...
    default:
        {
            ...
            sizebox_vis = !(mode == SIZE_MAXIMIZED || is_win_maximized());
 ...
 */
TadsCmdStat_t CHtmlDbgWatchWin::active_check_command(
    const check_cmd_info *info)
{
    if (exprwin_ != 0)
        return exprwin_->check_command(info);
    else
                                        ...
                                        const textchar_t *init_expr)
{
    ...
    wid = 500;
    ht = 250;
    rc.left = drc.left + (drc.right - drc.left - wid)/2;
    rc.right = rc.left + wid;
    rc.top = drc.top + (drc.bottom - drc.top - ht)/2;
    rc.bottom = rc.top + ht;
 ...
 */
void CHtmlDbgExprWin::draw_focus(int draw, HDC hdc)
{
    RECT rc;
    CHtmlRect text_area;
    HDC dc;
    ...
    /* if the focus area is scrolled off the screen above, don't draw it */
    if (text_area.bottom <= hdr_height_)
        return;
    ...
    /* inset it slightly so that it draws inside the area */
    InflateRect(&rc, -1, -1);

    /* draw it */
    dc = (hdc != 0 ? hdc : GetDC(handle_));
    DrawFocusRect(dc, &rc);
    if (hdc == 0)
        ReleaseDC(handle_, dc);
}
                                          ...
                                          int on_screen)
{
    ...
    /* the document canvas runs from 0 to our fixed width */
    long xl = 0;
    long xr = SCROLL_WIDTH * get_hscroll_units();

    /* if we want an on-screen area, limit the width to the client area */
    if (on_screen)
    {
        RECT rc;
        GetClientRect(handle_, &rc);
        rc.right = screen_to_doc_x(rc.right -= GetSystemMetrics(SM_CXVSCROLL));
        if (xr > rc.right)
            xr = rc.right;
    }
 ...
 */
TadsCmdStat_t CHtmlDbgExprWin::check_command(const check_cmd_info *info)
{
    ...
    case ID_EDIT_COPY:
        ...
        if (info->command_id == ID_EDIT_DELETE)
                                       ...
                                       const RECT *area_to_draw)
{
    ...
    FillRect(hdc, area_to_draw, GetSysColorBrush(COLOR_WINDOW));
...
void CHtmlDbgExprWin::scroll_focus_into_view()
{
    ...
    /*
     *   if there's a focus expression, it's the one to scroll into view;
     *   otherwise, scroll the current selection into view
     */
    target = (focus_expr_ != 0 ? focus_expr_ : selection_);

    /* if there's no target for scrolling, there's nothing to do */
    if (target == 0)
        return;
    ...
    item_top = target->get_y_pos();
    item_bottom = item_top + target->get_height();
...
{
    /*
     *   (We formerly removed the focus rectangle here with a call to
     *   draw_focus(FALSE, 0).  The focus rectangle uses xor drawing and
     *   hence we were worried that it wouldn't translate well during
     *   scrolling.  But on reconsideration this seems to be the wrong thing
     *   to do; the scrolled area simply keeps the focus rectangle as you'd
     *   expect, and we fill in the focus rectangle in the newly exposed area
     *   as necessary in the redraw.  Clipping during drawing ensures that
     *   areas not repainted during the scroll aren't XORed out.)
     */
}
...
{
    /*
     *   (We formerly had to put back the focus rectangle here with a call to
     *   draw_focus(TRUE, 0).  This wasn't the right thing to do, for reasons
     *   explained in notify_pre_scroll.
     */
                                 ...
                                 const textchar_t *relationship)
{
         ...
         cur = cur->get_next_expr())
    {
        ...
        if (get_strlen(cur->get_expr()) == exprlen
            ...
            && strcmp(relationship, cur->get_parent_rel()) == 0)
        {
...
void CHtmlDbgExprWin::do_resize(int mode, int x, int y)
{
    ...
    /* if our separator is off the screen, put it at the halfway mark */
    if (x > 5 && (sep_xpos_ < 5 || sep_xpos_ > x - 5))
        sep_xpos_ = x / 2;
                                  ...
                                  size_t panel_num)
{
    ...
    /* save the expression list, if desired */
    if (config_saves_expr_list())
    {
        /*
         ...
         *   only, since they're the only ones directly enterable by the
         *   user.
         */
        for (cur = first_expr_, cnt = 0 ; cur != 0 ;
             cur = cur->get_next_expr())
        {
            /*
                                  ...
                                  size_t panel_num)
{
    ...
    /* load the expression list, if desired */
    if (config_saves_expr_list())
    {
        /*
         *   build our element name - base it on the panel number, so that we
                              ...
                              DWORD __RPC_FAR *effect)
{
    ...
    /* assume we won't accept drops */
    *effect = DROPEFFECT_NONE;
    drop_ok_ = FALSE;

    /*
     *   if I don't have a blank entry line, or I'm single-entry, I'm
     *   read-only and thus can't accept drops
     */
 ...
 */
HRESULT STDMETHODCALLTYPE CHtmlDbgExprWin::DragOver(
    DWORD grfKeyState, POINTL pt, DWORD __RPC_FAR *effect)
{
    ...
    *effect = (drop_ok_ ? DROPEFFECT_COPY : DROPEFFECT_NONE);
    return S_OK;
}
 ...
 *   drop in this window
 */
HRESULT STDMETHODCALLTYPE CHtmlDbgExprWin::Drop(
    ...
    DWORD __RPC_FAR *effect)
{
    ...
    {
        /* get the text */
        textchar_t *buf = (textchar_t *)GlobalLock(medium.hGlobal);
...
void CHtmlDbgExpr::build_full_expr(CHtmlDbgExpr *parent)
{
    if (parent != 0)
    {
        /* start with the parent expression */
        fullexpr_.set("(");
        fullexpr_.append(parent->get_full_expr());
        fullexpr_.append(")");

        /* get the parent relationship */
        const char *pr = parent_relationship_.get();

        /*
         *   if it starts with '!', it's special - it means that what follows
         *   is the whole relationship string, without the sub-expression;
         *   otherwise it's an operator to which we append the sub-expression
         */
        if (pr != 0 && *pr == '!')
        {
            /* 'pr' is the entire selector expression */
            fullexpr_.append(pr + 1);
        }
        else
        {
            /* 'pr' is just an operator - append our subexpression */
            fullexpr_.append(pr);
    ...
    {
        /* there's no parent - my expression stands alone */
