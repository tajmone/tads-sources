...
private:
    ...
    /*
     *   arrays of our read/write button identifiers; the array index is the
     *   corresponding safety level setting
     */
    static int rbuttons_[];
    static int wbuttons_[];

    /* preferences object */
    CHtmlPreferences *prefs_;
};

int CHtmlDialogSafety::rbuttons_[] =
{
    IDC_RB_SAFETY0R,
    IDC_RB_SAFETY0R,
    IDC_RB_SAFETY2R,
...
int CHtmlDialogSafety::wbuttons_[] =
{
    IDC_RB_SAFETY0W,
    IDC_RB_SAFETY1W,
    IDC_RB_SAFETY1W,
...
void CHtmlDialogSafety::init()
{
    ...
    prefs_->get_file_safety_level(&saved_read_, &saved_write_);

    /* initialize the appropriate radio button for the current setting */
    CheckRadioButton(handle_, IDC_RB_SAFETY0R, IDC_RB_SAFETY4R,
                     rbuttons_[saved_read_]);
    CheckRadioButton(handle_, IDC_RB_SAFETY0W, IDC_RB_SAFETY4W,
                     wbuttons_[saved_write_]);
}
...
{
    int level, read = -1, write = -1;
    int ch;

    /* get the current button setting */
    for (level = 0 ; level < sizeof(rbuttons_)/sizeof(rbuttons_[0]) ; ++level)
    {
        /* if this button is checked, it's the new level */
        if (read < 0
            && IsDlgButtonChecked(handle_, rbuttons_[level]) == BST_CHECKED)
            read = level;
        if (write < 0
            && IsDlgButtonChecked(handle_, wbuttons_[level]) == BST_CHECKED)
            write = level;
    }

    /* if this is different from the button last selected, so note */
    ch = (read != saved_read_ || write != saved_write_);

    /* if saving, update the preferences */
    if (save && ch)
        prefs_->set_file_safety_level(read, write);
...
/* ------------------------------------------------------------------------ */
/*
 *   Network safety level dialog
 */

class CHtmlDialogNetSafety: public CTadsDialogPropPage
{
public:
    CHtmlDialogNetSafety(CHtmlPreferences *prefs)
    {
        /* save the preferences object */
        prefs_ = prefs;
    }
...
private:
    ...
    /* levels selected on initialization or last "apply" */
    int saved_client_level_;
    int saved_server_level_;

    /*
     *   array of our button identifiers; the array index is the
     *   corresponding safety level setting
     */
    static int cli_buttons_[], srv_buttons_[];

    /* preferences object */
    CHtmlPreferences *prefs_;
};

int CHtmlDialogNetSafety::cli_buttons_[] =
{
    IDC_RB_SAFETY0,
    IDC_RB_SAFETY1,
    IDC_RB_SAFETY2
};
int CHtmlDialogNetSafety::srv_buttons_[] =
{
    IDC_RB_SAFETY3,
    IDC_RB_SAFETY4,
    IDC_RB_SAFETY5
 ...
 *   initialize
 */
void CHtmlDialogNetSafety::init()
{
    /* note the initial button setting */
    prefs_->get_net_safety_level(&saved_client_level_, &saved_server_level_);

    /* initialize the appropriate radio buttons for the current settings */
    CheckRadioButton(handle_, IDC_RB_SAFETY0, IDC_RB_SAFETY2,
                     cli_buttons_[saved_client_level_]);
    CheckRadioButton(handle_, IDC_RB_SAFETY3, IDC_RB_SAFETY5,
                     srv_buttons_[saved_server_level_]);
}

/*
 *   handle a command
 */
int CHtmlDialogNetSafety::do_command(WPARAM id, HWND ctl)
{
    /* check the control */
    switch(id)
    {
 ...
 *   process notifications
 */
int CHtmlDialogNetSafety::do_notify(NMHDR *nm, int)
{
    switch(nm->code)
    {
    case PSN_APPLY:
        ...
        /* note the button as of this last save */
        prefs_->get_net_safety_level(
            &saved_client_level_, &saved_server_level_);
 ...
 *   check for changes
 */
int CHtmlDialogNetSafety::has_changes(int save)
{
    int i, cli_level, srv_level;
    int ch;

    /* get the current client button setting */
    for (i = 0 ; i < sizeof(cli_buttons_)/sizeof(cli_buttons_[0]) ; ++i)
    {
        /* if this button is checked, it's the new level */
        if (IsDlgButtonChecked(handle_, cli_buttons_[i]) == BST_CHECKED)
            break;
    }

    /* if this is different from the button last selected, so note */
    ch = ((cli_level = i) != saved_client_level_);

    /* check the server buttons */
    for (i = 0 ; i < sizeof(srv_buttons_)/sizeof(srv_buttons_[0]) ; ++i)
    {
        /* if this button is checked, it's the new level */
        if (IsDlgButtonChecked(handle_, srv_buttons_[i]) == BST_CHECKED)
            break;
    }

    /* if this is different from the button last selected, so note */
    ch |= ((srv_level = i) != saved_server_level_);

    /* if saving, update the preferences */
    if (save && ch)
        prefs_->set_net_safety_level(cli_level, srv_level);
                                           ...
                                           CHtmlWinWithPrefs *win)
{
    ...
    net_dlg = new CHtmlDialogNetSafety(this);
    ...
    /* set up the Network Safety Level page */
    psp[i].dwSize = sizeof(PROPSHEETPAGE);
    psp[i].dwFlags = PSP_USETITLE;
    psp[i].hInstance = CTadsApp::get_app()->get_instance();
    ...
    psp[i].dwFlags = PSP_USETITLE;
    psp[i].hInstance = CTadsApp::get_app()->get_instance();
    psp[i].pszTemplate = MAKEINTRESOURCE(DLG_NET_SAFETY);
    psp[i].pszIcon = 0;
    psp[i].pszTitle = MAKEINTRESOURCE(IDS_NET_SAFETY);
    net_dlg->prepare_prop_page(&psp[i]);
    ++i;
