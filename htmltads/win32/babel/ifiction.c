...
{
char *xml, buffer[2400], *aep=0, *mda=md, ebuffer[512];
...
{
 fprintf(stderr,"Error: XML header not found.");
 return;
}

xml=strstr(md,"<ifindex");
if (!xml) {
 fprintf(stderr,"Error: <ifindex> not found");
 return;
 }
while(xml && *xml)
{
...
static void ifiction_find_value(struct XMLTag *xtg, void *xti)
{
 ...
 if (gt->target && gt->output && strcmp(gt->output,gt->target)==0) gt->target=NULL;
