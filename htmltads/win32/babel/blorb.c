...
static char *blorb_get_story_format(void *blorb_file, int32 extent)
{
 ...
 for(j=0;TranslateExec[j];j+=2)
  if (blorb_get_chunk(blorb_file,extent, TranslateExec[j], &i, &i)) return TranslateExec[j+1];
 for(j=0;treaty_registry[j];j++)
 {
  char bb[5];
  static char fn[512];
  treaty_registry[j](GET_FORMAT_NAME_SEL,NULL,0,fn,512);
  for(i=0;i<4 && fn[i];i++) bb[i]=(char)toupper(fn[i]);
  while (i<4) bb[i++]=' ';
  bb[4]=0;
  if (blorb_get_chunk(blorb_file,extent,bb,&i, &i)) return fn;
 }
 return NULL;
}
