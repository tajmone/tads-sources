...
/* sound file playback completion message */
static const int HTMLM_SOUND_DONE = WM_USER + 3;
...
static const int HTMLM_BRING_TO_FRONT = WM_USER + 7;

/* last HTMLM message - subclasses can use this as their base */
static const int HTMLM_LAST = WM_USER + 7;
...
public:
    ...
    /*
     *   CHtmlWinWithPrefs interface implementation
     */
    ...
    void set_selected_text_color(int use_default_fg, HTML_color_t fg,
                                 int use_default_bg, HTML_color_t bg)
    {
        /* note the new colors */
        default_sel_fg_ = use_default_fg;
        default_sel_bg_ = use_default_bg;
        sel_fg_ = HTML_color_to_COLORREF(fg);
        sel_bg_ = HTML_color_to_COLORREF(bg);

        /* make sure we redraw with the new color scheme */
        inval();
    }
...
protected:
    ...
    /*
     *   run a 'find' command; shows the Find dialog, runs the search, and
     *   displays a message box if no match is found
     */
    ...
    void internal_set_input_color(COLORREF color);
...
protected:
    ...
    int default_sel_fg_, default_sel_bg_;
...
public:
    ...
    /*
     *   Check for an EOF input loop.  Each input request method should call
     *   this when it's about to return an EOF error code, to let us check to
     *   see if the program is looping on input requests after EOF.
     */
    void check_eof_input_loop();
...
protected:
     ...
     *   get_input_event, wait_for_keystroke, or similar method will return
     *   end of file.
     *
     *   EOF in this context means that the user has closed the main window,
     *   leaving the bytecode program running without a source of user input.
     *   We don't just kill the bytecode program at this point, because it
     *   could a legitimate reason to keep running briefly, such as to save
     *   files or otherwise clean up before terminating.  We relay the EOF
     *   status back to the bytecode program on any input request, such as a
     *   keystroke or command line input.
     */
    unsigned int eof_ : 1;
    ...
    /*
     *   Number of key/event input requests with the EOF flag set.  If the
     *   program makes too many calls for input after EOF, we'll assume that
     *   it's ignoring the EOF status, so we'll kill the program.  Some older
     *   games don't correctly handle the EOF error and just keep asking for
     *   input, so they get stuck in an input loop, spinning at 100% CPU on
     *   repeated attempts for input that never succeed.  This is especially
     *   pernicious because EOF means we no longer have a window, so there's
     *   no visible indication of what's wedged or what's eating all the CPU
     *   time.
     */
    int eof_input_counter_;
...
public:
    ...
    virtual oshtml_charset_id_t owner_get_default_charset() const
        { return oshtml_charset_id_t(ANSI_CHARSET, GetACP()); }
...
public:
    ...
    /* get/set the Web UI flag for the loaded game */
    int is_webui_game() const { return webui_game_; }
    void set_webui_game(int flag) { webui_game_ = flag; }
    ...
    virtual void skip_create_system_window()
    {
        ...
        CHtmlSys_framewin::skip_create_system_window();
    }
    ...
    /* are we running under the debugger? */
    int is_in_debugger() const { return in_debugger_; }
     ...
     *   it; if it's minimized, restore it to normal size
     */
    void show_normal();
...
public:
    ...
    virtual TadsCmdStat_t active_check_command(const check_cmd_info *) = 0;
    ...
    /* bring the window to the front */
    virtual void active_to_front() = 0;

    /*
     *   customize a tooltip text string; if this returns FALSE, we'll simply
     *   use the standard tooltip text string for the command instead
     */
    virtual int active_get_tooltip(TOOLTIPTEXT *ttt) { return FALSE; }

    /*
     *   Get the current selection, if appropriate.  Returns true and fills
     *   in the pointers if a selection range was returned, false if not.
     */
    ...
    virtual int get_sel_text(CStringBuf *buf, unsigned int flags,
                             size_t maxlen) = 0;
#define DSW_WORD_EXTEND   0x0001
#define DSW_EXPR_EXTEND   0x0002

    /*
     *   Set the selection range, if possible.  Returns true if the selection
     *   range was set, false if not.
     */
    virtual int set_sel_range(unsigned long sel_start,
                              unsigned long sel_end) = 0;

    /*
     *   Get the current selection range as a pair of spots.  A "spot" is a
     *   *handle* to a buffer character position, with the property that the
     *   window keeps a spot up to date whenever the buffer is edited.
     *   Calling this routine creates two spot objects, which must be deleted
     *   when no longer needed.
     */
    virtual int get_sel_range_spots(void **a, void **b) { return FALSE; }

    /* get the buffer position from a spot object */
    virtual unsigned long get_spot_pos(void *spot) { return 0; }

    /* delete a "spot" object */
    virtual void delete_spot(void *spot) { }

    /*
     *   Get the local key binding table name for this window type.  By
     *   default, windows use the Global key bindings.
     */
    virtual const textchar_t *active_get_key_table() { return "Global"; }

    /*
     *   Get my editor mode, if applicable.  By default, we return nothing;
     *   source editor windows override this to return the editor mode
     *   object.
     */
    virtual struct ITadsWorkbenchEditorMode *get_editor_mode() const
        { return 0; }

    /* get the name of the file the window is displaying, if any */
    virtual const textchar_t *active_get_filename() const { return 0; }

    /* get the text editor caret line and column, if applicable */
    virtual int active_get_caretpos(int *line, int *col) { return FALSE; }

    /* begin/end an Undo group */
    virtual void active_undo_group(int begin) { }

    /* is a Find command allowed in this window? */
    virtual int active_allow_find(int regex) { return FALSE; }

    /* is a Replace command allowed in this window? */
    virtual int active_allow_replace() { return FALSE; }
...
public:
    ...
    /*
     *   Get the selection range as a pair of spots.  HTML windows don't
     *   allow their contents to be edited, so a spot is effectively
     *   immutable - so we don't need to keep real handle-based spots.
     *   Instead, we can simply use a document position directly as a spot
     *   handle.
     */
    ...
    {
        if (html_panel_ != 0)
            SendMessage(html_panel_->get_handle(), WM_ENTERSIZEMOVE, 0, 0);
    }
    void do_exitsizemove()
    {
        if (html_panel_ != 0)
            SendMessage(html_panel_->get_handle(), WM_EXITSIZEMOVE, 0, 0);
    }

    /* these windows allow Find, but only for literal text */
    virtual int active_allow_find(int regex) { return !regex; }
    virtual int active_find_text(
        const textchar_t *txt,
        int exact_case, int wrap, int direction, int regex, int whole_word,
        unsigned long first_match_a, unsigned long first_match_b);
    ...
    virtual void active_to_front() { bring_owner_to_front(); }
...
/* ------------------------------------------------------------------------ */
/*
 *   Basic CHtmlSysWinGroup implementation for debugger windows
 */
class CHtmlSysWinGroupDbg: public CHtmlSysWinGroup
{
    /* get the default character set */
    oshtml_charset_id_t get_default_win_charset() const
...
class CHtmlSys_dbglogwin: public CHtmlSys_dbgwin, public CHtmlSysWinGroupDbg
{
public:
...
/* ------------------------------------------------------------------------ */
/*
 *   Debugger system-specific (i.e., Windows-specific) window interface.  All
 *   debugger content windows (source file, stack, debug log, trace history,
 *   search results) implement this interface.
 *
 *   For our CHtmlSysWin-based windows, the "content window" is the HTML
 *   panel, which is embedded in a frame/container window.  The container
 *   window implements the MDI or SDI frame.  This arrangement isn't
 *   necessary in general; it's just what's convenient for CHtmlSysWin
 *   windows.  For Scintilla windows, in contrast, the frame window hosts the
 *   IDebugSysWin interface.
 */
class IDebugSysWin: public IDebugWin
{
public:
    /* get the window handle */
    virtual HWND idsw_get_handle() = 0;

    /* get the frame window handle */
    virtual HWND idsw_get_frame_handle() = 0;

    /* bring the window to the front */
    virtual void idsw_bring_to_front() = 0;

    /* select the current line */
    virtual void idsw_select_line() = 0;

    /* note formatting changes in the debugger */
    virtual void idsw_note_debug_format_changes(
        HTML_color_t bkg_color, HTML_color_t text_color,
        int use_windows_colors, int use_windows_bgcolor,
        HTML_color_t sel_bkg_color, HTML_color_t sel_text_color,
        int use_windows_sel_colors, int use_windows_sel_bgcolor) = 0;

    /* mark the file as untitled */
    virtual void idsw_set_untitled() = 0;

    /*
     *   Get the configuration path and title for this type of window.  These
     *   are the values stored under the "mdi win list" and "mdi win title"
     *   keys in the configuration.  For actual source-file windows, these
     *   are the actual filename.  For tool windows, these are of the form
     *   ":name", where "name" is a symbolic name for the window type.
     */
    virtual const textchar_t *idsw_get_cfg_path() = 0;
    virtual const textchar_t *idsw_get_cfg_title() = 0;

    /*
     *   Get this window as a CHtmlDbgSubWin object.  Some IDebugSysWin
     *   objects exhibit the CHtmlDbgSubWin interface, others don't; this
     *   will return the other interface if available, or null if not.
     */
    virtual class CHtmlDbgSubWin *idsw_as_dbgsubwin() { return 0; }

    /* get this window as a text editor window, if applicable */
    virtual class CHtmlSys_dbgsrcwin *idsw_as_srcwin() { return 0; }
};


/* ------------------------------------------------------------------------ */
/*
 *   IDebugSysWin implementation based on CHtmlSysWin_win32
 */
class CDebugSysWin: public CHtmlSysWin_win32, public IDebugSysWin
{
public:
    CDebugSysWin(class CHtmlFormatter *formatter,
                 class CHtmlSysWin_win32_owner *owner,
                 int has_vscroll, int has_hscroll,
                 class CHtmlPreferences *prefs)
    ...
    void idw_add_ref() { AddRef(); }
    void idw_release_ref() { Release(); }
    void idw_close() { close_window(TRUE); }
    int idw_query_close(struct ask_save_ctx *ctx) { return TRUE; }
    int idw_is_dirty() { return FALSE; }
    void idw_show_source_line(int linenum);
    void idw_reformat(const CHtmlRect *new_scroll_pos);
    void idw_post_load_init();
    virtual CHtmlSysWin *idw_as_syswin() { return this; }

    HWND idsw_get_handle() { return get_handle(); }
    HWND idsw_get_frame_handle() { return get_frame_handle(); }
    void idsw_bring_to_front() { bring_to_front(); }
    void idsw_note_debug_format_changes(
        HTML_color_t bg, HTML_color_t txt, int usewinclr, int usewinbg,
        HTML_color_t selbg, HTML_color_t seltxt,
        int usewinsel, int usewinselbg)
    {
        note_debug_format_changes(bg, txt, usewinclr, usewinbg,
                                  selbg, seltxt, usewinsel, usewinselbg);
    }
    void idsw_set_untitled() { }
    void idsw_select_line() { }

    /*
     *   Set the body colors.  Setting the default does nothing, since we
     *   want to use the explicitly-set debugger colors.  However, we do
     *   allow explicit overrides for non-default settings from the text.
     */
    virtual void set_html_bg_color(HTML_color_t color, int dflt)
    {
        if (!dflt)
            CHtmlSysWin_win32::set_html_bg_color(color, FALSE);
    }
    virtual void set_html_text_color(HTML_color_t color, int dflt)
    {
        if (!dflt)
            CHtmlSysWin_win32::set_html_text_color(color, FALSE);
    }
 ...
 */
class CHtmlSysWin_win32_dbglog: public CDebugSysWin
{
public:
    ...
    virtual const textchar_t *idsw_get_cfg_path() { return ":log"; }
...
protected:
     ...
     */
    void show_error_line(int by_mouse);
...
protected:
    ...
    virtual const char *xtra_lib_credits() const { return ""; }

    /* get extra library copyrights to show at the start of the list */
    virtual const char *xtra_copyrights() const { return ""; }
