                                      ...
                                      CHtmlTableMetrics *metrics)
{
    ...
    if ((metrics->no_break_ || metrics->no_break_sect_) && len_ != 0)
    {
        ...
        nxt = os_next_char(charset, p, rem);
        rem -= (nxt - p);
        p = nxt;
    }
                                          ...
                                          CHtmlLineBreak *break_pos)
{
    ...
    if (break_pos->no_break_ && cur_cnt < len_)
    {
        ...
        prev_char = *p;
        p = os_next_char(charset, p, len_ - cur_cnt);
        cur_cnt = p - txt_;
    }
    ...
    for ( ; cur_cnt < len_ ;
         p = os_next_char(charset, p, len_ - cur_cnt), cur_cnt = p - txt_)
    {
                                       ...
                                       const textchar_t *last_break_pos)
{
    ...
    {
        const textchar_t *nxt = os_next_char(charset, p, rem);
        rem -= (nxt - p);
        p = nxt;
    }
...
void CHtmlDispText::find_trailing_whitespace(CHtmlDisp_wsinfo *info)
{
    ...
    for (p = txt_, rem = len_ ; rem != 0 ;
         nxt = os_next_char(charset, p, rem), rem -= nxt - p, p = nxt)
    {
    ...
    CHtmlPoint pos) const
{
    ...
    {
        /* note our character set */
        oshtml_charset_id_t charset = font_->get_charset();

        /* find byte length of the next character */
        size_t clen = os_next_char(charset, txt_ + fit_cnt, len_ - fit_cnt)
                      - (txt_ + fit_cnt);

        /* check to see if we're halfway into it */
        if (pos.x >
            (pos_.left
             + win->measure_text(font_, txt_, fit_cnt, 0).x
             + win->measure_text(font_, txt_ + fit_cnt, clen, 0).x / 2))
        {
                                             ...
                                             CHtmlLineBreak *break_pos)
{
    ...
    /* ...but if something breakable follows, don't prevent a break there */
    break_pos->no_break_ = FALSE;
                                      ...
                                      class CHtmlTableMetrics *metrics)
{
    ...
    /*
     *   if breaking before this point is allowed (i.e., we're following a
     *   space), and I'm not zero-width (in which case I'm actually a
     *   no-break flag), clear the leftover
     */
    if (len_ != 0
        && !metrics->no_break_
        && !metrics->no_break_sect_
        && is_space(metrics->last_char_))
        metrics->clear_leftover();
...
CHtmlDispSpace::CHtmlDispSpace(CHtmlSysWin *win, CHtmlSysFont *font,
                               const textchar_t *txt, size_t len,
                               unsigned long txtofs, int wid, int pre)
    : CHtmlDispTextSpecial(win, font, txt, len, txtofs)
{
    CHtmlPoint siz;
    CHtmlFontDesc desc;

    /* remember the preformatted flag */
    pre_ = pre;
...
{
    /*
     *   if we're in preformatted text, we can't break before or after a
     *   typographical space
     */
    if (pre_)
    {
        break_pos->last_char_ = 0;
        break_pos->no_break_ = TRUE;
        break_pos->note_non_ws();
        return 0;
    }
                                   ...
                                   int /*clicked*/)
{
    ...
    /* figure the size we need to display our text */
    text_len = win->measure_text(font_, num_.get(),
                                 get_strlen(num_.get()), 0).x
               + win->measure_text(font_, "  ", 2, 0).x;

    /* draw our number as ordinary text, right-justified in our rectangle */
    win->draw_text(FALSE, pos_.right - text_len, pos_.top, font_,
